...
# Map
Presented up front is the __*final*__ mapping product on which the rest of the document is based.The map shows the ORMGP jurisdiction discretized into sub-watersheds each approximately ~10km² in area. Using the mouse to click on any sub-watershed will return a number of properties of the selected sub-watershed. In the figure below, sub-watersheds are colour-coded according to their degree of impervious cover.  The menu in the top-right corner provides options for canopy coverage, weltland coverage, open water coverage and permeability.
...
The ORMGP jurisdiction, which is over three-million hectares, has been subdivided into sub-watersheds (approximately 10 km² in area) to provide a basis for hydrometeorological data analysis. Each sub-watershed has a defined topological order in which headwater sub-watersheds can easily be mapped to subsequent downstream sub-watersheds, and so on until feeding the great lakes. The intent is to deem these sub-watersheds suitable for climatological and water budget analyses. Below is a description of the derivation the v.2020 OWRC 10km² sub-watershed map and its derivatives including:
...
Next, the hydrologically-corrected digital elevation model (HDEM) is further processed to derive a number of metrics aggregated at the sub-watershed scale.

# Digital Elevation Model
Ground surface elevations were collected from the 2006 version of the provincial (Ministry of Natural Resources) DEM. This is a 10x10 m DEM derived from 1:10,000 scale OBM base mapping based on photogrammetic elevation points and 5 m contours where the photogrammetic elevation points did not exist. An up-scaled 50x50 m Digital Elevation Model (DEM) is produced by merging the tiles shown below:
...
Elevation data were up-scaled by taking the average of known elevations occurring within every 50x50 m cell. The resulting grid is a 5000x5000 50 m-uniform cell grid, with an upper-left-corner origin at (E: 545,000, N: 5,035,000) NAD83 UTM zone 17.
